...
export type SortTasksOptions<T extends object> = {
  tasks: T[]; // Lista de tarefas que será ordenada
  direction?: 'asc' | 'desc'; // Direção da ordenação: crescente ou decrescente (opcional)
  field?: keyof T; // Qual campo da tarefa será usado para ordenar (opcional)
};

export function sortTasks<T extends object>({
  field = 'startDate' as keyof T, // Se o campo não for informado, usamos 'startDate' como padrão
